...
def downvote(request, target):
    ...
    return redirect(reverse('view:show_video', kwargs={'target': target}))
...
def invalid_streams(request):
    ...
    return render(request, 'view/invalid.html', context={'stream_list': active_streams})
...
def show_video(request, target):
    ...
    return render(request, 'view/play.html', context=ctx)
